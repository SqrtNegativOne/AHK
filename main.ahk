...
; ::;dp::window.PDFViewerApplication.pdfViewer.container.style.filter = "invert(1)"

::;dp::
{
    A_Clipboard := 'window.PDFViewerApplication.pdfViewer.container.style.filter = "invert(1)"'
    Send "^v{Enter}"
    Sleep 50
    Send "^+i"
}
...
; abc
Space & d::{ ; PDF invert color / dark mode ; Doesn't work.
	Send "^+i"
	Sleep 10
	Send "^+k"
    A_Clipboard := 'window.PDFViewerApplication.pdfViewer.container.style.filter = "invert(1)"'
	; Send('window.PDFViewerApplication.pdfViewer.container.style.filter = "invert(1)"')
    Send "^v"
	Send "{Enter}"
    Sleep 800
    Send "^+i"
...
::\lim::\lim\limits_{{}x → 0{}}
;::\int::\displaystyle∫\,dx{left 4} ; Goodbye.
